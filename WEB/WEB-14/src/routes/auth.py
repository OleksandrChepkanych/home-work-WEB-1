...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    The signup function creates a new user in the database.
        ...
        A new contact record is created with this information and returned to the client.
...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    :return: A token, but the refresh_token function returns a new token
    :doc-author: Trelent
    """
...
async def refresh_token(credentials: HTTPAuthorizationCredentials = Security(security), db: Session = Depends(get_db)):
    ...
    The refresh_token function is used to refresh the access token.
        ...
        or refreshing of tokens then an HTTPException will be raised.

    :param credentials: HTTPAuthorizationCredentials: Get the token from the request header
    :param db: Session: Access the database
    :return: An object with the access_token, refresh_token and token type
    :doc-author: Trelent
    """
